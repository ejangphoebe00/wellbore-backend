...
class CraneUser(db.Model):
    ...
    DeactivateAccount = db.Column(db.SMALLINT, db.CheckConstraint('0 <= DeactivateAccount <= 1'),nullable=False)
    ActivationChangeComment = db.Column(db.NVARCHAR(255),nullable=True)
    ActivationChangeDate = db.Column(db.DateTime,nullable=True)
    CredentialsSent = db.Column(db.SMALLINT, db.CheckConstraint('0 <= CredentialsSent <= 1'),nullable=True)
    UserOnlineStatus = db.Column(db.SMALLINT, db.CheckConstraint('0 <= UserOnlineStatus <= 1'),nullable=True)
